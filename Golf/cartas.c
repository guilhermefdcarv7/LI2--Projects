...
// CC = 2
void biscar(struct Mesa *mesa){ // Função que bisca uma carta do baralho de biscar e coloca-a no topo da pilha de descarte.
    if(mesa->baralho.quantidade > 0){ // Se ainda existirem cartas no baralho de biscar...
        struct Carta carta = carta_de_cima(mesa->baralho); // Obtém a carta de cima da pilha de biscar.
        mesa->baralho = retira_carta_de_cima(mesa->baralho); // Retira a carta do topo do baralho de biscar.
        mesa->descarte = add_carta(mesa->descarte, carta); // Coloca a carta biscada no topo da pilha de descarte.
    }
}

// CC = 3
void jogar_carta(struct Mesa *mesa, char op_jogo){
    int numero_da_pilha = (op_jogo - '1'); // Converte a opção escolhida pelo usuário (caracter '1' a '7') para um inteiro correspondente ao número da pilha do tabuleiro (0 a 6).
    if(mesa->tabuleiro[numero_da_pilha].quantidade != 0){ // Se a pilha do tabuleiro escolhida pelo usuário não estiver vazia...
        struct Carta carta_da_pilha_escolhida = carta_de_cima(mesa->tabuleiro[numero_da_pilha]); // Obtém a carta que está no topo da pilha do tabuleiro escolhida pelo utilizador.
        struct Carta carta_do_descarte = carta_de_cima(mesa->descarte); // Obtém a carta que está no topo da pilha de descarte.
        if(pode_jogar(carta_da_pilha_escolhida, carta_do_descarte) == 1){ // Se puder jogar a carta do topo da pilha do tabuleiro escolhida pelo usuário com a carta do topo da pilha de descarte...
            mesa->tabuleiro[numero_da_pilha] = retira_carta_de_cima(mesa->tabuleiro[numero_da_pilha]); // Retira a carta do topo da pilha do tabuleiro escolhida pelo usuário.
            mesa->descarte = add_carta(mesa->descarte, carta_da_pilha_escolhida); // Coloca a carta do topo da pilha do tabuleiro escolhida pelo usuário no topo da pilha de descarte.
        }
    }
}
...
struct Mesa prepara_mesa(){ // Função que prepara a mesa para o início do jogo, criando o baralho de biscar, as 7 pilhas do tabuleiro e a pilha de descarte.
    ...
    for(int i = 0; i < 7; i++){ // Percorre as 7 colunas do tabuleiro.                                                                               ////
        mesa.tabuleiro[i] = pilha_vazia(); // Inicializa cada coluna do tabuleiro como vazia.                                                     //
        for(int j = 0; j < 5; j++){ // Coloca 5 cartas em cada uma das 7 pilhas do tabuleiro.                                                     // 
            struct Carta carta_tirada = carta_de_cima(mesa.baralho); // Obtém a carta que está no topo do baralho de biscar.                      // Distribuição das cartas do baralho de biscar para as 7 pilhas do tabuleiro, colocando 5 cartas em cada pilha.
    ...
    mesa.descarte = pilha_vazia(); // Inicializa a pilha de descarte como vazia.
    struct Carta primeira_carta_biscar = carta_de_cima(mesa.baralho); // Obtem a carta do topo do baralho de biscar para começar a pilha de descarte.
    mesa.baralho = retira_carta_de_cima(mesa.baralho); // Remove essa carta do baralho de biscar.
    mesa.descarte = add_carta(mesa.descarte, primeira_carta_biscar); // Coloca essa carta na pilha do descarte.
